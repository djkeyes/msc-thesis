...
static void defaultDistribs(Mat& means, vector<Mat>& covs,
                            int type = CV_32FC1) {
...
static void generateData(Mat& data, Mat& labels, const vector<int>& sizes,
                         const Mat& _means, const vector<Mat>& covs,
                         int dataType, int labelType) {
  vector<int>::const_iterator sit = sizes.begin();
  int total = 0;
  for (; sit != sizes.end(); ++sit) total += *sit;
  ...
  for (int i = 0; i < _means.rows; i++) means[i] = _means.row(i);
...
static int maxIdx(const vector<int>& count) {
  int idx = -1;
  int maxVal = -1;
...
static bool getLabelsMap(const Mat& labels, const vector<int>& sizes,
  ...
  if (checkClusterUniq) {
    for (size_t i = 0; i < buzy.size(); i++)
      if (!buzy[i]) return false;
  }

  return true;
}

static bool calcErr(const Mat& labels, const Mat& origLabels,
                    const vector<int>& sizes, float& err,
                    bool labelsEquivalent = true,
  ...
  bool isFlt = labels.type() == CV_32FC1;
  if (!labelsEquivalent) {
    if (!getLabelsMap(labels, sizes, labelsMap, checkClusterUniq)) return false;

    for (int i = 0; i < labels.rows; i++)
      if (isFlt)
        err += labels.at<float>(i) != labelsMap[(int)origLabels.at<float>(i)]
                   ? 1.f
                   : 0.f;
      else
        err +=
            labels.at<int>(i) != labelsMap[origLabels.at<int>(i)] ? 1.f : 0.f;
  } else {
    for (int i = 0; i < labels.rows; i++)
      if (isFlt)
...
bool test_approxkmeans_clustering() {
  ...
  vector<int> sizes(sizesArr,
                    sizesArr + sizeof(sizesArr) / sizeof(sizesArr[0]));
  Mat means;
  vector<Mat> covs;
  ...
  kmeans(data, 3, bestLabels, TermCriteria(TermCriteria::COUNT, iters, 0.0), 0,
  ...
  kmeans(data, 3, bestLabels, TermCriteria(TermCriteria::COUNT, iters, 0.0), 0,
         KMEANS_USE_INITIAL_LABELS, noArray());
  if (!calcErr(bestLabels, labels, sizes, err, false)) {
