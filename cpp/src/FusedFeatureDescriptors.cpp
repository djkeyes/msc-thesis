...
void NearestDescriptorAssigner::detectAndCompute(InputArray image_,
                                                 InputArray mask,
  ...
  if (!useProvidedKeypoints) {
    ...
    // -bucket descriptors and linear search through adjacent buckets (slow for
    // dense)
    ...
    vector<vector<list<const KeyPoint*>>> buckets(
        bucket_rows,
        vector<list<const KeyPoint*>>(bucket_cols, list<const KeyPoint*>()));
    ...
    for (const auto& element : curSceneCoords.coords) {
      ...
      for (int bucket_row = std::max(0, row - 1);
           bucket_row <= std::min(bucket_rows - 1, row + 1); ++bucket_row) {
        for (int bucket_col = std::max(0, col - 1);
             bucket_col <= std::min(bucket_cols - 1, col + 1); ++bucket_col) {
