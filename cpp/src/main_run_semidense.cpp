...
int main(int argc, char** argv) {
  ...
  named_args.add_options()(HELP_FLAG.c_str(), "produce help message")(
      OUTPUT_PATH_FLAG.c_str(), po::value<string>(), "specify output path");
  ...
  // boost program_options require positional args to have a corresponding named
  // arg
  ...
  if (vm.count(HELP_FLAG) || vm.count(OUTPUT_PATH_FLAG) == 0) {
    cout << "Usage: " << argv[0] << " --" << OUTPUT_PATH_FLAG
         << "=<output-path> <input-filepath1> <input-filepath2> ..." << endl;
    cout << named_args << "\n";
    return 1;
  }
  ...
  for (const auto& input_path : input_paths) {
    ...
    int num_segments = max(1, map_gen->getNumImages() / frames_per_segment);
