...
static void generateRandomCenter(const std::vector<Vec2f>& box, float* center,
	...
	for (j = 0; j < dims; j++)
		center[j] = ((float) rng * (1.f + margin * 2.f) - margin)
				* (box[j][1] - box[j][0]) + box[j][0];
}

static void generateCentersPP(const Mat& _data, Mat& _out_centers, int K,
		RNG& rng, int trials) {
	throw std::logic_error("KMeans++ is not implemented for ApproxKMeans! "
...
public:
	ApproxKMeansDistanceComputer(double *_distances, int *_labels,
			...
			distances(_distances), labels(_labels), data(_data), flann_index(
					_flann_index) {
	}
...
double approx_kmeans(InputArray _data, int K, InputOutputArray _bestLabels,
	...
	if (flags & CV_KMEANS_USE_INITIAL_LABELS) {
		CV_Assert(
				(best_labels.cols == 1 || best_labels.rows == 1)
		...
		for (iter = 0;;) {
			...
			if (iter == 0 && (a > 0 || !(flags & KMEANS_USE_INITIAL_LABELS))) {
				...
				if (num_empty_clusters > 0) {
					std::cout << num_empty_clusters << " empty clusters!"
							<< std::endl;
					...
					// To this end:
					//   1. put clusters in a heap sorted by size, to efficiently fetch max element. We only need the top-most heaps.
					//   2. create a smaller heap for each largest cluster, sorted by distance
					//   3. for each point, add it to the heap of its cluster if it's in one of the larger clusters

					auto comp =
							[&counters](const int& first, const int& second) {
								return counters[first] - counters[second] <= 0;
							};
					std::priority_queue<int, std::vector<int>, decltype(comp)> cluster_size_queue(
							comp);
					for (int k = 0; k < K; k++) {
						if (counters[k] > 0) {
					...
					while (num_removable_points < num_empty_clusters) {
						...
						num_removable_points += largest_clusters.size()
								* (counters[last] - next_count);
					}
					unsigned int num_points_to_leave = (num_removable_points
							- num_empty_clusters) / largest_clusters.size()
							+ next_count;

					auto point_comp =
							[&dists](const int& first, const int& second) {
								return dists.at<double>(first) -dists.at<double>(second) <= 0;
							};
					std::map<int,
							std::priority_queue<int, std::vector<int>,
									decltype(point_comp)>> removable_points;
					...
					for (int k = 0; k < K; k++) {
						...
						if (candidate_point_iter->second.size()
								== num_points_to_leave) {
			...
			// TODO: is FLANN already multithreaded? If so, this might just be adding overhead
